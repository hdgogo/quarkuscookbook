package org.example.basedi.lock;
...
public class AccountBalanceService {
    ...
    @Lock(value = Lock.Type.READ, time=5, unit = TimeUnit.SECONDS)  // 设置读锁
    public int get() {
        // 模拟耗时的操作
        try {
            Thread.sleep(80);
    ...
    public void deduct(int value) {
        // 先将accountBalance的值存入tempValue变量
        int tempValue  = accountBalance;
        Log.infov("start deduct, balance [{0}], deduct value [{1}]", tempValue, value);
